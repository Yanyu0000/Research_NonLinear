...
i <- 1
while(i <= n) {
  x1_r <- runif(1, 1, 4)            # Random price
  x2_r <- runif(1, -2, 1)           # Normalized calorie
  x3_r <- runif(1, -2, 1)           # Normalized size
...
############################## Plot Level Curves ##############################
grid_size <- 100
calorie_values <- seq(-2, 1, length.out = grid_size)
size_values <- seq(-2, 1, length.out = grid_size)
...
calculate_utility <- function(calorie, size, beta_cal, beta_size, beta_interact, organic_effect = 1) {
  quadratic_component <- beta_cal * (calorie^2) + beta_size * (size^2) + beta_interact * calorie * size
...
contour_levels <- seq(-2, 0, by = 0.25)
...
# Use sample splitting to address bias. First train on partition 1 (P1) and predict on P2. Then, invert it
...
h_ApplePrice <- 0.2 * sd(data_juice$ApplePrice)
h_BerryPrice <- 0.2 * sd(data_juice$BerryPrice)
